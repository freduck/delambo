...
$query = "SELECT * FROM exam WHERE subject =3 LIMIT $limit OFFSET $offset";
$result = $conn->query($query);

$total_query = "SELECT COUNT(*) AS total_questions FROM exam WHERE subject =3";
...
// // Display questions
echo "<div class='question'><form action='submit.php' method='post'>";
$c=0;
while ($row = $result->fetch_assoc()) {
  ...
  echo "<input type='hidden' name='question_id' value='".$row['id']."'>";
   echo "<input type='hidden' name='subject_id' value='".$row['subject']."'>";
     echo "<input type='hidden' name='subject' value='c.c.a'>";
...
<p><a href="result.php?subject=3">End Exam</a></p>
</div>
<h3 id='answered'>Answered Questions</h3>
<ul class='answered-question'>

    <?php
$select=$conn->query("SELECT * FROM user_responses WHERE user_id='$user_id' AND subject_id=3");

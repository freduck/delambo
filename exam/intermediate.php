...
$query = "SELECT * FROM exam WHERE subject =6 LIMIT $limit OFFSET $offset";
$result = $conn->query($query);

$total_query = "SELECT COUNT(*) AS total_questions FROM exam WHERE subject =6";
...
while ($row = $result->fetch_assoc()) {
  ...
  echo "<input type='hidden' name='question_id' value='".$row['id']."' id='qid'>";
   echo "<input type='hidden' name='subject_id' value='".$row['subject']."' id='sid'>";
     echo "<input type='hidden' name='subject' value='intermediate' id='subject'>";
  
}
echo "<input type='button' value='Confirm Your Answer' name='submit' id='confirm'>";
echo "</form>";
?>
...
<p><a href="result.php?subject=6">End Exam</a></p>
</div>
<br>
<hr><br>

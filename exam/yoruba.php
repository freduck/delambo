...
$query = "SELECT * FROM exam WHERE subject =8 LIMIT $limit OFFSET $offset";
$result = $conn->query($query);

$total_query = "SELECT COUNT(*) AS total_questions FROM exam WHERE subject =8";
...
while ($row = $result->fetch_assoc()) {
  ...
  echo "<input type='hidden' name='question_id' value='".$row['id']."'>";
   echo "<input type='hidden' name='subject_id' value='".$row['subject']."'>";
     echo "<input type='hidden' name='subject' value='yoruba'>";
...
<p><a href="result.php?subject=8">End Exam</a></p>
</div>
<br>
<hr><br>

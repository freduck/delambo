...
$query = "SELECT * FROM exam WHERE subject =7 LIMIT $limit OFFSET $offset";
$result = $conn->query($query);

$total_query = "SELECT COUNT(*) AS total_questions FROM exam WHERE subject =7";
...
while ($row = $result->fetch_assoc()) {
  ...
  echo "<input type='hidden' name='question_id' value='".$row['id']."'>";
   echo "<input type='hidden' name='subject_id' value='".$row['subject']."'>";
     echo "<input type='hidden' name='subject' value='business'>";
...
<p><a href="result.php?subject=7">End Exam</a></p>
</div>
<br>
<hr><br>
...
<ul class='answered-question'>
    ...
    <?php
$select=$conn->query("SELECT * FROM user_responses WHERE user_id='$user_id' AND subject_id=7");
